...
**Dữ liệu luôn mới, không tích luỹ**: mỗi lượt thu thập GHI ĐÈ toàn bộ `listings[]`
của file vùng bằng đúng tin tìm được trong lượt đó — không giữ/merge tin của các lượt
trước. Nhà đất bán/gỡ tin rất nhanh nên tin của tuần trước gần như chắc chắn đã hết
còn bán; giữ lại chỉ tạo ảo giác "nhiều dữ liệu" trong khi phần lớn đã lỗi thời. Vì
vậy tổng số tin có thể dao động (kể cả giảm) giữa các lượt — đó là kết quả đúng.
...
## 2. Chống trùng lặp (deduplication) — trong phạm vi một lượt thu thập
...
- Trùng `dedup_key` trên **cùng site** hoặc **khác site**, tìm được TRONG CÙNG LƯỢT
  thu thập → giữ **1 bản** (bản mô tả đầy đủ nhất), đánh dấu `duplicate_count` = số
  lần xuất hiện trong lượt đó. Vì mỗi lượt ghi đè toàn bộ dữ liệu (xem §0), không so
  sánh/dedup với các lượt trước.
- `duplicate_count` cao (cùng tin xuất hiện nhiều lần trong một lượt tìm kiếm) là
  **tín hiệu xấu** (hàng khó bán, có thể vướng pháp lý) → trừ điểm độ phù hợp (§5).
...
Ghi ĐÈ `data/<region-slug>.json` theo `data/schema.json` mỗi lượt chạy (không tích luỹ —
xem §0), gồm: `region`, `market_bands[]` (dải giá theo quận/khu + nguồn), `listings[]`
(đã lọc & dedup nội bộ lượt này), `sources_used[]`, `notes`. `id` đánh số lại từ 1.
