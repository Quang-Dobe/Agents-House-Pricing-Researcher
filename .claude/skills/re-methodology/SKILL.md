...
## Dữ liệu luôn mới — KHÔNG tích luỹ qua các lượt (tuyệt đối)

- Mỗi lượt thu thập **GHI ĐÈ TOÀN BỘ** `listings[]` của file vùng bằng đúng những tin
  tìm được TRONG LƯỢT NÀY. Không merge, không giữ lại tin của lượt trước.
- Lý do: tin BĐS bán/gỡ rất nhanh — tin của 1-2 tuần trước gần như chắc chắn đã hết còn
  bán. Giữ chúng lại chỉ tạo ảo giác "nhiều dữ liệu" trong khi phần lớn đã lỗi thời —
  đi ngược nguyên tắc trung thực của repo này.
- Số lượng tin sau mỗi lượt vì vậy có thể NHỎ hơn hẳn lượt trước — đó là kết quả đúng,
  không phải lỗi. Không cố "bù" cho tin bị mất bằng cách nới lỏng filter hay ép số.
...
## Dedup (chỉ trong phạm vi lượt chạy này)

`dedup_key` = chuẩn hoá(đường/khu + phường/quận + diện tích ±1m² + giá ±10tr), bỏ dấu,
lowercase. Cùng một tin xuất hiện qua nhiều truy vấn/site khác nhau TRONG LƯỢT NÀY →
giữ bản đầy đủ nhất, tăng `duplicate_count`. Không so với dữ liệu của các lượt trước
(đã bị ghi đè — xem mục "Dữ liệu luôn mới" ở trên). `duplicate_count ≥ 3` = tín hiệu
xấu (rao dai không bán được) → trừ điểm suitability.
